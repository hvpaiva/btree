...
data Node n a
  = Subtree (BTree n a) a (BTree n a)
  | Subtree' (BTree n a) a (BTree n a) a (BTree n a)

-- | A B-Tree. This is an internal implementation, clients will use @Tree@ instead.
--
...
data BTree n a where
  Branch :: Node n a -> BTree ('Succ n) a
  Leaf :: BTree 'Z a
...
branch' :: BTree n a -> a -> BTree n a -> a -> BTree n a -> BTree ('Succ n) a
branch' a b c d e = Branch (Subtree' a b c d e)

-- | Inserts an element into a B-Tree.
insert :: forall a. Ord a => a -> Tree a -> Tree a
insert x (Tree tree) = insert' tree Tree $ \a b c -> Tree (branch a b c)
    where
      insert' :: forall n t. BTree n a -> Keep t n a -> Push t n a -> t
      ...
      insert' (Branch n) = i n
        where 
          ...
          i (Subtree' a b c d e) keep push = select' x b d xltb xeqb xbtw xeqd xgtd
            where
              _ = keep :: BTree m a -> t
              _ = push :: BTree m a -> a -> BTree m a -> t
...
data Shrunk (n :: Natural) a where
  H :: BTree n a -> Shrunk ('Succ n) a

-- | Delete an element from a B-Tree.
delete :: forall a. Ord a => a -> Tree a -> Tree a
delete x (Tree tree) = find tree Tree shrink
  where
    shrink :: forall n. Shrunk n a -> Tree a
    shrink (H t) = Tree t

    find :: forall n t. BTree n a -> Keep t n a -> Pull t n a -> t
    ...
    find (Branch (Subtree a b c)) keep pull = select x b xltb xeqb xgtb
      where
        ...
        mrgl :: forall p. ('Succ p ~ n) => Shrunk p a -> a -> BTree p a -> t
    ...
    replace :: forall n t. BTree n a -> Keep (a -> t) n a -> Pull (a -> t) n a -> t -> t
    ...
    mrg2r :: forall p t. Keep t ('Succ p) a -> Pull t ('Succ p) a -> BTree p a -> a -> Shrunk p a -> t
    ...
    mrg3r :: forall p t. Keep t ('Succ p) a -> BTree p a -> a -> BTree p a -> a -> Shrunk p a -> t
...
search :: forall a. Ord a => a -> Tree a -> Maybe a
search x (Tree tree) = search' tree 
  where 
    search' :: forall n. BTree n a -> Maybe a
    search' Leaf = Nothing
...
draw :: forall a. Show a => Tree a -> String
draw (Tree tree) = draw' tree
  where
    draw' :: BTree n a -> String
